...
**기술 선택 근거**
- **Qdrant** — 단일 인스턴스에서 컬렉션 단위로 시장 평가 데이터(`market_eval`)와 DNA 롤모델 데이터(`dna_rolemodel`)를 격리 운영할 수 있습니다.
...
### 보고서 생성 에이전트
`allRejected` 플래그에 따라 투자 보고서(투자 추천 순위) 또는 반려 사유서(서칭한 스타트업 전체가 보류된 경우) 전용 System Prompt를 분기 적용하여 보고서를 생성합니다. 투자 리스크 관련 정보를 프롬프트 레벨에서 필수 강조 항목으로 지정했습니다.
...
* **단정적 합불(Pass/Fail) 지양, 데이터 기반 '우선순위(Ranking)' 제공**
  * AI의 단편적인 투자 결정(환각 및 편향)을 방지합니다.
  * 가중 합산(Team 35%, Market 25%, Tech 25%, DNA 15%) 기반의 1~10위 랭킹표를 제공하여, 객관적인 1차 필터링과 포트폴리오 조망을 돕습니다.

* **반도체 산업 특수성을 반영한 '성공 DNA 유사도' 지표 도입**
  * 막대한 초기 자본과 인프라가 필요한 반도체 씬의 특성을 반영했습니다.
  * 글로벌 탑티어(NVIDIA, Qualcomm, AMD)의 성공 궤적을 Vector DB(Qdrant)화 하여, 타겟 기업의 방향성이 성공 사례와 얼마나 일치하는지 코사인 유사도 수치로 직관적으로 보여줍니다.

* **확증 편향을 막는 '핵심 리스크(Risk)' 강제 추출**
  * 프롬프트를 통해 장점뿐만 아니라 기업별 약점(예: 파운드리 파트너십 부재, 엑싯 경험 부족 등)을 필수적으로 서술하도록 강제했습니다.
  * 이를 통해 후속 투자 실사(Due Diligence) 단계에서 집중 검증해야 할 명확한 가이드라인을 제시합니다.

* **합산 총점 기반의 과락 시스템으로 '숨은 원석' 발굴**
  * 단일 항목 미달로 인한 억울한 조기 탈락을 방지하기 위해 합산 총점 필터링을 적용했습니다.
  * 창업 초기라 팀 역량은 다소 부족하더라도, 기술력과 시장성이 압도적인 고잠재력 스타트업을 놓치지 않고 발굴할 수 있도록 설계했습니다.
...
설계 초기, State 스키마에 실제 VC들이 판단 근거로 삼는 핵심 재무 지표들을 반영하려 했지만, 대부분 폐기했습니다.
...
추후 프로젝트를 진행하는 과정에 있어서는 도메인에 맞춰 데이터를 먼저 수집 후, 기획을 구체화하는 방식으로 고도화해야겠다고 생각했습니다.

---

## 2. 보고서 생성 에이전트에서만 StateGraph를 사용한 것에 대해서

설계 초기, 팀원 간에 전체 파이프라인을 어떤 아키텍처와 컨벤션으로 연결할지에 대한 명확한 사전 합의가 부족해서, 두 가지 구현 방식이 하나의 파이프라인 안에 혼재하게 되었습니다.

앞단의 `searchCorp`, `investDecision` 에이전트는 팀원들이 각자 익숙한 방식에 따라 순수 Python 함수 호출로 먼저 구현했습니다. 반면, 이후에 구현된 보고서 생성 에이전트 단계에서는 새로운 노드를 추가하면서 LangGraph의 StateGraph를 도입했습니다. 

마지막 노드라는 위치 덕분에 독립적인 호출과 테스트가 가능했고, 추후 보고서 작성의 분기 처리도 쉽게 확장할 수 있는 구조를 열어두었다는 점은 긍정적이었습니다. 하지만 전체 시스템 관점에서는 순수 함수 흐름에서 갑자기 그래프 아키텍처로 넘어가는 다소 어색한 구조가 되었습니다. 이로 인해 기존의 함수 안에서 강제로 그래프를 호출하는 불안정한 연결부가 생기고 말았습니다.
...
처음부터 파이프라인 전체에 대한 코드 컨벤션을 설계했다면 노드 간 연결이 선언적으로 명확해졌을 것 같습니다. 

---

## 3. 병렬 처리의 성패는 올바른 자원 관리에 달려있다.

설계 초기, 에이전트의 작업 처리 속도를 극대화하기 위해 Supervisor 패턴을 활용한 Graph 아키텍처 기반의 병렬 처리를 기획하고 구축했지만, 이 병렬 처리 구조는 메모리 이슈로 인해 실패했습니다.
...
## 4. 시스템 통합 비용의 오판과 프롬프트 의존도의 함정

프로젝트 초기, 전체 아키텍처와 워크플로우를 먼저 설계한 뒤 파트별로 분업하여 개발한 후 고도화 해나가면 작업 효율을 극대화할 수 있을 것이라 판단했습니다.

하지만 현실은 예상과 달랐습니다. 각자 개발한 모듈을 하나로 병합(Merge)하고 파이프라인을 엮어내는 과정에서 데이터 정합성 문제와 예기치 못한 충돌이 연이어 발생했고, 이를 디버깅하며 통합하는 데 많은 시간이 소요되었습니다.

이런 이유로 정작 이 프로젝트의 핵심인 '반도체 스타트업 세부 평가 항목'을 정교하게 다듬을 시간이 부족해졌습니다. 시장성, 기술력, 창업자 역량이라는 3대 핵심 지표를 도메인 지식에 기반한 구체적인 채점 룰이나 세부 가중치로 세분화하지 못한 채 서비스 동작을 위한 구현 단계에 집중해야했습니다.

이로 인해 구체적인 평가 로직을 시스템에 내재화하지 못하고, LLM이 자체적으로 텍스트를 추론해 뽑아주는 점수와 판단에 전적으로 의존하는 구조가 되어버렸습니다. 범용적인 LLM의 시각에만 기대다 보니 원했던 수준의 성능과 정확도를 얻지 못하는 큰 아쉬움이 남았습니다.
